...
| 6 | [#5081: Integrate EIA 923 Stocks Data](https://github.com/catalyst-cooperative/pudl/issues/5081) | [catalyst-cooperative/pudl](https://github.com/catalyst-cooperative/pudl) (Python) | [PR #5431](https://github.com/catalyst-cooperative/pudl/pull/5431), inject `report_year` in the stocks extract, `_core_eia923__yearly_fuel_stocks` Dagster asset (wide monthly coal/oil/petcoke to tall monthly records), resource and field metadata, alembic migration, dbt validations, unit test | 🔄 **In review**, opened as a draft 2026-07-24 at the design checkpoint; @zaneselvans gave the go-ahead 2026-07-30, so I built the full implementation (asset, metadata, migration `0636821e0069`, dbt; materialized 17,758 rows), left `census_division_and_state` unconstrained, and marked it ready with `Closes #5081` |
...
**pudl #5431, reply posted (2026-08-02 check).** [Posted the drafted reply](https://github.com/catalyst-cooperative/pudl/pull/5431#issuecomment-5159154796) to @zaneselvans confirming which parts of `pytest-ci` were run locally (unit tests, Dagster materialize, `dbt_helper validate`; not the full integration suite, which needs an ETL data environment this workspace doesn't have).

**pudl #5431, @zaneselvans follow-up (2026-08-04 check).** After the reply above, @zaneselvans came back asking to actually run `pixi run pytest-ci` locally before merging, to check for unexpected side effects, not just report which pieces were already run. This needs my own machine (real `$PUDL_INPUT`/`$PUDL_OUTPUT`), so it is not something this check-in session can do. Action needed from me: run the full `pytest-ci` suite locally and reply with the result before this can move to merge.
...
branch's own CI confirms it end to end. No further response from @zaneselvans yet; still awaiting a
