...
| 6 | [#5081: Integrate EIA 923 Stocks Data](https://github.com/catalyst-cooperative/pudl/issues/5081) | [catalyst-cooperative/pudl](https://github.com/catalyst-cooperative/pudl) (Python) | [PR #5431](https://github.com/catalyst-cooperative/pudl/pull/5431), inject `report_year` in the stocks extract, `_core_eia923__yearly_fuel_stocks` Dagster asset (wide monthly coal/oil/petcoke to tall monthly records), resource and field metadata, alembic migration, dbt validations, unit test | 🟢 **Approved, merge imminent**, opened as a draft 2026-07-24 at the design checkpoint; @zaneselvans gave the go-ahead 2026-07-30, so I built the full implementation (asset, metadata, migration `0636821e0069`, dbt; materialized 17,758 rows), left `census_division_and_state` unconstrained, and marked it ready with `Closes #5081`. Review round resolved (petcoke units to short tons per the EIA-923 instructions, `W` redaction test, seed CSV sort), and EIA confirmed on 2026-08-05 that the raw sheet's "Thousand Barrels" label for petcoke was their error, validating the units decision; the last gate @zaneselvans asked for is **cleared**: merged current `main` and ran the full local `pytest-ci` on 2026-08-05, 961 unit + 450 integration tests passed, 0 failures, 93% coverage. Kept mergeable through two upstream conflict rounds since (`673af05aa4`, then `ad606f5101` across the v2026.8.0 release boundary). **@aesharpe approved on 2026-08-07**, said she will merge once the team's release finishes today |
...
| **[6](#cycle-6)** | pudl #5081 | energy-data pipeline (Python) | 🟢 approved, merge imminent |
