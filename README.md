...
Also verified what `c452435` actually changed, since it went up unreviewed by me: six files, the three cross-validation tasks (DRSM, biomimicry, MeSH descriptors) each setting `eval_splits` and `train_split` to `evaluation` and keeping the `DatasetDict` key as `evaluation` in `dataset_transform`, plus the three matching descriptive-stats JSON files renaming their top-level `train` key to `evaluation`. FoS is untouched, correctly, since it is multi-label and does not take the cross-validation path. A short nudge asking @Samoed or @KennethEnevoldsen to release the workflow run is drafted and held for me to post. The honest caveat that goes with it: that commit has still never been executed by anything, locally or in CI, so approving the run is also the first real test of it.
