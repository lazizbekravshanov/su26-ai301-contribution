...
- [ ] **On a busy upstream, re-check mergeability while the PR waits.** A green, mergeable PR can silently go CONFLICTING hours later when `main` moves. _(Cycle 6: pudl landed three commits the same afternoon and flipped #5431 to CONFLICTING; caught it on a routine status check, not from any notification.)_
